##CIUSuite_detect##
##written by Joseph D. Eschweiler, finalized on 3/25/2015 ##
...
answer = raw_input("Axis titles are in Trap Collision Voltage (V) and Drift Time (ms) by default. Please type y to continue or n to change these settings: ")
lower = answer.lower()
assert lower == 'y' or lower == 'n', "Your input must be y or n"
if lower == 'n':
    titlex = raw_input("Please title your X axis:") #For our lab normally Trap Collision Voltage
    titley = raw_input("Please title your Y axis:") #Normally Drift time or CCS
elif lower == 'y':
    titlex = "Trap Collision Voltage (V)"
    titley = "Drift Time (ms)"

window = raw_input("Please choose a Savitzky-Golay Smoothing Window(must be odd integer greater than 2, we recommend 5 to start) ")
try:
    window = int(window)
...
threshold = float(raw_input("Please enter the signal intensity threshold(e.g. '80'):  "))
assert 0 < threshold < 100, "You must enter a value between 0 and 100"
threshold = float(threshold)/100
scalef = raw_input("Please enter a scaling factor for your data(e.g. '0.5', '1', or '2'):  ")
try:
    scalef = float(scalef)
except (TypeError, ValueError):
